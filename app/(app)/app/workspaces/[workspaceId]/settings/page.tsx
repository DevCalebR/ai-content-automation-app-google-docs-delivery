...
export default async function SettingsPage({
  ...
  const deliveryReadinessItems = getDeliveryReadinessItems({
    activeMode,
    integrationStatus: integration?.status,
    oauthConnected,
    oauthRefreshReady,
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
      <Panel className="p-7">
        ...
        <div className="mt-6">
          <Badge>
            {authorization.isOwner ? "OWNER" : authorization.membership.role}
          </Badge>
        </div>
        <div className="mt-8">
          {authorization.isOwner ? (
            ...
            <div className="rounded-[1.75rem] border border-dashed border-[var(--line)] bg-white/60 p-6 text-sm text-[var(--ink-soft)]">
              Only workspace owners can change settings. You can still create
              briefs, run generations, and review saved outputs.
            </div>
          )}
        </div>
      </Panel>
      <Panel className="p-7">
        ...
        <div className="mt-5 rounded-[1.75rem] border border-[var(--line)] bg-white/80 p-5">
          <p className="font-medium text-[var(--ink)]">Readiness checklist</p>
          <p className="mt-2 text-sm leading-7 text-[var(--ink-soft)]">
            Completed runs can be delivered from the results page as soon as
            every item below is ready.
          </p>
                ...
                <div className="flex items-start gap-3">
                  <span
                    ...
                    className={`mt-1 h-2.5 w-2.5 rounded-full ${
                      item.ready ? "bg-[var(--success)]" : "bg-[var(--danger)]"
                    }`}
                  />
                  <div>
                    <p className="font-medium text-[var(--ink)]">
        ...
        <div className="mt-5 grid gap-5">
              ...
              {authorization.isOwner ? (
                oauthReady ? (
                  ...
                  <p className="mt-4 text-sm leading-7">
                    ...
                    before using My Drive delivery.
                  </p>
                )
              ) : (
                <p className="mt-4 text-sm leading-7">
                  Only workspace owners can connect a Google account for
                  delivery.
                </p>
              )}
            </div>
          ...
          <div className="rounded-[1.75rem] border border-[var(--line)] bg-white/80 p-5">
            ...
            <div className="mt-4 rounded-[1.5rem] border border-[var(--line)] bg-[var(--panel-strong)] p-4 text-sm text-[var(--ink-soft)]">
              <p className="font-medium text-[var(--ink)]">
                Workspace delivery service account
              </p>
              {serviceAccountEmail ? (
                <>
                  <p className="mt-2 leading-7">
                ...
                <p className="mt-2 leading-7">
                  The service account email is not available in this environment
                  because the server credentials are not configured.
                </p>
              )}
            </div>
            <div className="mt-4">
              {authorization.isOwner ? (
                ...
                <div className="rounded-[1.5rem] border border-dashed border-[var(--line)] bg-white/60 p-5 text-sm text-[var(--ink-soft)]">
                  Only workspace owners can change the service-account delivery
                  folder.
                </div>
              )}
            </div>
        ...
        {connectionState?.folderId ? (
          <div className="mt-5 rounded-[1.75rem] border border-[var(--line)] bg-[var(--panel-strong)] p-4 text-sm text-[var(--ink-soft)]">
            ...
            <p className="mt-3 leading-7">
              This is the folder completed runs will use when someone chooses
              Deliver to Google Docs from results or history.
            </p>
          </div>
        ) : null}
      </Panel>
